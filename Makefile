...
run-test:
	...
	@npm run system-test -- --baseurl $(BASEURL) --test-type $(TEST_TYPE) --python $(PYTHON)
...
ifdef SAUCE_USER_NAME
	...
	BASEURL=$(BASEURL) TEST_TYPE=remote PYTHON=$(PYTHON) $(MAKE) run-test
else
	$(MAKE) start-selenium
	$(MAKE) sdist
	@echo 'Starting system integration tests locally...'
	BASEURL=$(BASEURL) TEST_TYPE=local PYTHON=$(PYTHON) $(MAKE) run-test || (docker rm -f $(SERVER_NAME); -kill `cat SELENIUM_PID`; rm SELENIUM_PID; exit 1)
